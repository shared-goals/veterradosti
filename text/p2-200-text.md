...
Wiki – это же единственная версия правды.
Истины, в которую нереально вместить многообразие эмоций и взглядов разных [темпераментов](p1-020-call.md#mbti_personalities), [культур](p1-050-country.md#happiness_in_russia) и [поколений](p1-030-time.md#scientific_approach).
...
Поэтому мне ближе [открытые и некоммерческие](p2-170-opensource.md#associations_with_open_source) Тексты, объединенные Общей Целью нахождения в Радости и опубликованные в [Git](#git).
...
Инструкция с иллюстрациями перенесена в [README файл нашего хранилища в GitHub](https://github.com/bongiozzo/whattodo).

Если у Вас ещё нет учётной записи GitHub – потребуется зарегистрироваться.

Это несложно — на момент написания этих строк регистрация не вызывала сложностей, нужно только решить головоломку, для подтверждения своей человечности!
:-)

Хотя, сдаётся мне, что головоломка уже устарела и легко проходится [агентами ИИ](p2-160-routine.md#robots_are_coming).

Зачем нужно регистрироваться?

Свобода свободой, но не сказать, что я буду счастлив, если каждый начнёт редактировать наш Текст, как вздумается!
:-)

Но никто не может запретить скопировать открытый проект к себе и, взяв только нужное, начать создавать свою версию.
В терминологии Git это называется Fork – ответвление.
И для этого потребуется учётная запись.

И после запуска останется только писать свой оригинальный Текст – [сопутствующая рутина публикации уже автоматизирована](p2-160-routine.md#routine_as_disease) и [банальные стартовые тезисы уже выписаны](p1-010-happiness.md#cornerstone_of_text).

## VS Code и другие редакторы текста {#vscode}

Создание Текста в нашем мозгу процесс тонкий и зависящий от множества сопутствующих нюансов, сформированных за время жизни.
Кому то нравится писать пером на бумаге, кто-то предпочитает для этого использовать печатную машинку, а наиболее очевидный выбор в наше время – Word или Google Doc.

Мой выбор опять же обусловлен [призванием программиста](p1-020-call.md#follow_your_calling).

Весь процесс, от планирования структуры Текста до его публикации, можно делать в одном удобном, [открытом](p2-170-opensource.md#freedom_of_choice) и бесплатном редакторе [Visual Studio Code](https://code.visualstudio.com/).

Можно редактировать Текст в оффлайне, сохраняя изменения на локальный диск и, по завершению работы, синхронизироваться с хранилищем, сохраняя историю изменений.

[Многочисленные расширения редактора](https://marketplace.visualstudio.com/vscode) помогут в написании Текста – подсветят ошибки и покажут, как он будет выглядеть.
...
Любые вопросы можно задать мне в группе [канала](https://t.me/bongiozzo_public) или адресовать [ИИ-помощнику](p3-links.md).
